...
T_COLOR = pygame.Color(255, 255, 255)
...
class MainGame:
    ...
    def start_game(self):
        ...
        while True:
            ...
            self.blit_blast()
            # 文字显示
            MainGame.window.blit(self.text(' '*10 + '敌方剩余坦克数量：{0}'.format(len(MainGame.enemy_list)) + ' '*10 + '分数：{0}'.format(MainGame.score) + ' '*10 + '死亡了{}次'.format(MainGame.death_times)), dest=(10, 10))
            # 获取事件
            self.get_event()
            # 坦克移动
            if MainGame.my_tank:
    ...
    # 生成我方坦克
    def creat_mytank(self, num):
        if num == 1:
            if MainGame.my_time == 0:
                MainGame.my_tank = MyTank(screen_width/2, screen_height/2)
                MainGame.death_times += 1
                MainGame.my_time = 20
            else:
                MainGame.my_time -= 1
        else:
    ...
    def creat_wall(self):
        ...
        wall = Wall1(0, screen_height - wall.rect.height)
        MainGame.wall_list.append(wall)
        wall = Wall1(screen_width - wall.rect.width, 0)
        MainGame.wall_list.append(wall)
        wall = Wall1(screen_width - wall.rect.width, screen_height - wall.rect.height)
        MainGame.wall_list.append(wall)
        wall = Wall2(0, screen_height/2)
        MainGame.wall_list.append(wall)
        wall = Wall2(screen_width - wall.rect.width, screen_height/2)
        MainGame.wall_list.append(wall)
        for i in range(6):
            wall = Wall2(i*60+120, screen_height/2 - wall.rect.height*2)
            MainGame.wall_list.append(wall)
        for i in range(4):
            wall = Wall2(wall.rect.width*3, i*wall.rect.width+screen_height/2)
            MainGame.wall_list.append(wall)
        for i in range(4):
            wall = Wall2(wall.rect.width*6, i*wall.rect.width+screen_height/2)
            MainGame.wall_list.append(wall)
        for i in range(10):
            wall = Wall(i*wall.rect.width, screen_height/2 - wall.rect.height)
            MainGame.wall_list.append(wall)
...
class Tank(BaseItem):
    ...
    def hit_wall(self):
        for wall in MainGame.wall_list:
            if isinstance(wall, Wall1) or isinstance(wall, Wall2):
...
class Wall(BaseItem):
    ...
    def __init__(self, left, right):
        self.image = pygame.image.load('img/Resources/grass.gif')
...
class Wall1(Wall):
    ...
    def __init__(self, left, right):
        super(Wall1, self).__init__(left, right)
        self.image = pygame.image.load('img/Resources/walls.gif')

    def wall_bullet(self):
        super(Wall1, self).wall_bullet()
        self.hp = 3
...
class Wall2(Wall):
    ...
    def __init__(self, left, right):
        super(Wall2, self).__init__(left, right)
        self.image = pygame.image.load('img/steels.gif')
        self.hp = 20

    # def wall_bullet(self):
    #     super(Wall2, self).wall_bullet()
    #     self.hp = 3
