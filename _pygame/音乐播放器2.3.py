...
class MainMusicPlayer:
    ...
    def choose_play_methods(self):
        ...
        if method == '0':
            ...
            print('请输入合适的数！')
            return self.choose_play_methods()

    # 播放
    def play(self):
        if self.play_methods == 0:
            if self.loops == 0:
                if not Music.get_busy():
                    Music.display(loops=self.loops)
            elif type(self.loops) is int and self.loops > 0:
                if not Music.get_busy():
                    Music.display(loops=self.loops)
    ...
    def set_volume(self):
        ...
        try:
            volume = float(volume)
            if 0 <= volume <= 100:
                Music.set_volume(num=volume / 100)
