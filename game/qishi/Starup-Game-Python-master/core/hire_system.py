...
class HireSystem(object):
    ...
    def start_hire(self):
        ...
        ''
        print
        ','.join(map(lambda x: x.name, self.coders)) + u'带着发家致富的梦想加入的团队。'
        keyboard.next()
    ...
    def hire(self):
        print
        u"你可以最多可以雇佣%s名员工，请谨慎选择:" % (self.max_coders)
        keyboard.next()
        ...
        while len(self.coders) < self.min_coders:
            print
            u'你至少需要%s名员工加入你的团队!' % (self.min_coders)
            self._hire()
    ...
    def _hire(self):
        ...
        '------'
        print
        u"%s: %s 薪水： %s" % (coder.job, coder.name, coder.salary)
        if keyboard.confirm():
            self.coders.append(coder)
            self.avaliable_coders.remove(coder)
            print
            u"%s加入了你的团队。" % (coder.name)
