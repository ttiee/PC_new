            ...
            if rev["message_type"] == "private":  # 私聊
                ...
                if group == 643272204:
                    if "[CQ:at,qq=469784630]" in rev["raw_message"]:
                        if bu:
                            bu = False
                            send_msg({'msg_type': 'group', 'number': group, 'msg': '已关闭[CQ:face,id=189]'})
                        else:
                            bu = True
                            send_msg({'msg_type': 'group', 'number': group, 'msg': '已开启[CQ:face,id=189]'})
                    if bu:
                        mag = sayings()
                        send_msg({'msg_type': 'group', 'number': group, 'msg': mag})
                    # qq = rev['sender']['user_id']
                ...
                elif "[CQ:at,qq=469784630]" in rev["raw_message"]:
